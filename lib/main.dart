...
class BrandCard extends StatelessWidget {
  ...
  final double opacity; // allow more solid cards when needed
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        ...
        // ✅ FIXED overlay: simple ceremony (no CustomPaint), guaranteed to render on web.
        if (showGraduation)
          FadeTransition(
            opacity: fadeAnim,
            child: Stack(
              children: [
                Positioned.fill(child: Container(color: Colors.black.withOpacity(0.82))),
                Positioned.fill(
                  child: Center(
                    child: ConstrainedBox(
                      ...
                      child: Padding(
                        ...
                        child: LayoutBuilder(
                          builder: (context, constraints) {
                            ...
                            final h = desired.clamp(520.0, 720.0);

                            return BrandCard(
                              // ✅ more solid so it’s readable
                              opacity: 0.40,
                              padding: const EdgeInsets.all(18),
                              child: SizedBox(
                                width: double.infinity,
                                height: h,
                                child: SimpleCeremony(
                                  name: widget.name,
...
/// ✅ Ceremony that is “boringly reliable” on Flutter Web.
class SimpleCeremony extends StatelessWidget {
  final String name;
  final List<String> promises;
  final VoidCallback onEnter;

  const SimpleCeremony({
    super.key,
    required this.name,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const Icon(Icons.verified_rounded, color: NatterBrand.yellow, size: 72),
        const SizedBox(height: 14),
        Text(
          name,
          textAlign: TextAlign.center,
        ...
        child: Center(
          child: ConstrainedBox(
            ...
            child: const BrandCard(
              child: Text(
                'Parent page is included in this file.\nNext: we’ll polish layout + wire controls into behaviour.',
