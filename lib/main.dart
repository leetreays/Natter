...
class ParentExploreJourneyScreen extends StatelessWidget {
  const ParentExploreJourneyScreen({
    super.key,
    required this.child,
      ...
      child: Column(
        ...
        children: [
          ...
          NatterScreenHeader(
            ...
            subtitle:
                'Choose an area when you want to understand a little more.',
            compact: true,
          ),
...
DateTime _mostRecentSundayEvening() {
  final now = DateTime.now();
...
String _weeklyNoteText({
  ...
  if (quiet == 0 && guidance == 0 && connection == 0) {
    return 'This week felt calm overall.';
  }

  if (guidance > 0 && quiet == 0) {
    return 'This week brought a few learning moments.';
  }

  if (quiet > 0 && guidance == 0) {
    return 'This week included a few quiet-time reminders.';
  }

  if (connection > 0 && guidance == 0 && quiet == 0) {
    return 'This week included some positive connection moments.';
  }
...
_childSnapshotSection(
  signalsStream: signalsStream,
  childDocStream: childDocStream,
),
...
String _smoothedBanner(String activeBanner) {
  ...
  if (activeBanner == 'pause' || activeBanner == 'none') {
  _displayedBanner = activeBanner;
  _bannerShownAt = DateTime.now();
  _bannerTimer?.cancel();
  return _displayedBanner;
...
Future<TimeOfDay?> _showNatterWorldTimeSheet({
  required BuildContext context,
...
class _NatterWorldTimeSheet extends StatefulWidget {
  const _NatterWorldTimeSheet({
  ...
  @override
  State<_NatterWorldTimeSheet> createState() =>
      _NatterWorldTimeSheetState();
}

class _NatterWorldTimeSheetState
    extends State<_NatterWorldTimeSheet> {
  late int _selectedHour;
  late int _selectedMinute;
  late bool _selectedIsPm;
  ...
  Color get _accent => widget.isStart
      ? NatterBrand.pink
      : NatterBrand.yellow;

  NatterGlowTone get _glow => widget.isStart
      ? NatterGlowTone.protect
      : NatterGlowTone.hope;
  ...
  void initState() {
    ...
    _selectedIsPm =
        widget.initialTime.period == DayPeriod.pm;
  ...
  void _beginManualEntry() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _hourTextController.selection = TextSelection(
        baseOffset: 0,
        extentOffset:
            _hourTextController.text.length,
      );
    });
  }
  ...
  bool _applyManualEntry() {
    final hour = int.tryParse(
      _hourTextController.text.trim(),
    );

    final minute = int.tryParse(
      _minuteTextController.text.trim(),
    );
